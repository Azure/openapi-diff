...
const yaml_1 = require("./lib/ref/yaml");
...
const data_store_1 = require("./lib/data-store/data-store");
...
async function legacyMain(autorestArgs) {
    ...
    const dataStore = new data_store_1.DataStore();
    let config = {};
    try {
        config = await legacyCli_1.CreateConfiguration(currentDirUri, dataStore.GetReadThroughScope(new file_system_1.RealFileSystem()), autorestArgs);
    }
    catch (e) {
    ...
    if (autorestArgs[0] === "init") {
        const clientNameGuess = (config["override-info"] || {}).title ||
            yaml_1.Parse(await uri_1.ReadUri(config["input-file"][0])).info.title;
...
~~~ yaml
${yaml_1.Stringify(config).replace(/^---\n/, "")}
~~~
...
`.replace(/~/g, "`"));
    ...
    if (autorestArgs[0] === "init-cli") {
        const args = [];
        for (const node of jsonpath_1.nodes(config, "$..*")) {
            const path = node.path.join(".");
            const values = node.value instanceof Array
                ? node.value
    ...
    const api = new autorest_core_1.AutoRest(new file_system_1.RealFileSystem());
    ...
    api.ClearFolder.Subscribe((_, folder) => (outstanding = outstanding.then(async () => {
        try {
    ...
    for (const arg of autorestArgs) {
        ...
        rawValue =
            !isNaN(parseFloat(rawValue)) && rawValue.includes(".")
                ? `'${rawValue}'`
                : rawValue;
        const value = yaml_1.Parse(rawValue);
        result.rawSwitches[key] = value;
        result.switches.push(jsonpath_1.CreateObject(key.split("."), value));
    }
    return result;
}
...
async function currentMain(autorestArgs) {
    ...
    if (!args.rawSwitches["message-format"] ||
    ...
    const api = new autorest_core_1.AutoRest(new file_system_1.EnhancedFileSystem(configuration_1.MergeConfigurations(...args.switches)["github-auth-token"] ||
        process.env.GITHUB_AUTH_TOKEN), uri_1.ResolveUri(currentDirUri, args.configFileOrFolder || "."));
    ...
    if (config.HelpRequested) {
        ...
        const helpArtifacts = artifacts.sort((a, b) => a.uri === b.uri
            ? a.content > b.content
                ? 1
                : -1
            : a.uri > b.uri
        ...
        for (const artifact of artifacts) {
            await uri_1.WriteString(artifact.uri, artifact.content == null ? "" : artifact.content);
        }
    }
...
async function resourceSchemaBatch(api) {
    ...
    for (const batchConfig of config.GetNestedConfiguration("resource-schema-batch")) {
        ...
        for (const eachFile of batchConfig["input-file"]) {
            ...
            const instance = new autorest_core_1.AutoRest(new file_system_1.RealFileSystem(), config.configFileFolderUri);
...
async function batch(api) {
    ...
    for (const batchTaskConfig of config.GetEntry("batch")) {
        const isjson = args.rawSwitches["message-format"] === "json" ||
